// Vercel Serverless Function: /api/sitemap-blog
// Lists every published blog post's real URL (/blog/:id).

export const config = { runtime: 'nodejs' };

const SUPABASE_URL = process.env.VITE_SUPABASE_URL || process.env.SUPABASE_URL;
const SUPABASE_ANON_KEY = process.env.VITE_SUPABASE_ANON_KEY || process.env.SUPABASE_ANON_KEY;
const SITE_URL = 'https://www.sriganganagarjobs.in';

export default async function handler(req: any, res: any) {
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    res.status(500).send('Not configured');
    return;
  }
  try {
    const apiUrl = `${SUPABASE_URL}/rest/v1/blog_posts?select=id,created_at&order=created_at.desc&limit=500`;
    const response = await fetch(apiUrl, {
      headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${SUPABASE_ANON_KEY}` },
    });
    const rows = await response.json();
    const posts = Array.isArray(rows) ? rows : [];

    const urls = posts.map((p: any) => `
  <url>
    <loc>${SITE_URL}/blog/${p.id}</loc>
    <lastmod>${new Date(p.created_at).toISOString().split('T')[0]}</lastmod>
    <changefreq>monthly</changefreq>
    <priority>0.7</priority>
  </url>`).join('');

    const xml = `<?xml version="1.0" encoding="UTF-8"?>
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls}
</urlset>`;

    res.setHeader('Content-Type', 'application/xml; charset=utf-8');
    res.setHeader('Cache-Control', 'public, max-age=1800, s-maxage=3600');
    res.status(200).send(xml);
  } catch (err: any) {
    res.status(500).send('Error generating sitemap');
  }
}
